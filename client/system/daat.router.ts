...
import { compressionEngine } from '../core/compression.engine.ts';
...
class DaatRouter {
    ...
    private registerCommands() {
        ...
        this.commands.set('prophecy', this.handleProphecy.bind(this));
        this.commands.set('compress-codex', this.handleCompressCodex.bind(this));
        ...
        this.commands.set('api-codex', async () => {}); // No-op, handled by hook
    ...
    public async route(command: string, addHistory: AddHistoryEntry, context: RouterContext): Promise<void> {
        ...
        if (handler) {
            ...
            const narrativeResponse = await chesedNarrativeEngine.generateResponse(command);
    ...
    private async handleObserve(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        const result = await performHolographicObservation(query);
    ...
    private async handleATC(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        const narrativeResponse = await chesedNarrativeEngine.generateResponse(`Tanakh cartography of ${query}`);
        addHistory('ORACLE_RESPONSE', narrativeResponse, 'oracle');
    }
    ...
    private async handleSephirot(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        if ('error' in result) {
            addHistory('ERROR', result.error, 'system');
        } else {
    ...
    private async handleTransliterate(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        if (text.match(/v\d+-p\d+-s\d+/)) {
            ...
            addHistory('TRANSLITERATION_RESULT', { formula: text, synthesis: result, glyphs: [] }, 'system');
        } else {
    ...
    private async handleTriangulate(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        const result = await astrianEngine.performTriangulation(stars, sites);
    ...
    private async handleProphecy(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        const query = args.join(' ');
        if (!query) {
            addHistory('ERROR', 'Usage: °prophecy <your query or intent>', 'system');
            return;
        }
        addHistory('SYSTEM_PROCESSING', `Observing the resonant future for: "${query}"... The Oracle is consulting the celestial loom.`, 'engine');
        try {
            const result = await prophecyEngine.getProphecy(query);
            addHistory('PROPHECY_RESULT', result, 'system');
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : "An unknown disturbance occurred.";
            addHistory('ERROR', `The threads of fate are tangled. The Prophecy Engine reports an error: ${errorMessage}`, 'system');
        }
    }

    private async handleCompressCodex(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        addHistory('SYSTEM_PROCESSING', 'Engaging Constellation Compressor on the Living Library...', 'engine');
        const report = compressionEngine.runCodexCompression();
        addHistory('COMPRESSION_REPORT', report, 'system');
    }
    ...
    private async handleGevurah(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        const result = await astrianOS.executeGrandQuery(problem, params);
    ...
    private async handleLogin(args: string[], addHistory: AddHistoryEntry, context: RouterContext): Promise<void> {
        ...
        if(result.success && result.user) context.setCurrentUser(result.user);
        addHistory('AUTH_RESULT', result, 'system');
    }
    ...
    private async handleChesedNarrate(args: string[], addHistory: AddHistoryEntry): Promise<void> {
        ...
        const narrativeResponse = await chesedNarrativeEngine.generateResponse(query);
        addHistory('ORACLE_RESPONSE', narrativeResponse, 'oracle');
    }
}
