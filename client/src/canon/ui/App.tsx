...
// FIX: Corrected import path for useAstrianSystem hook.
import { useAstrianSystem } from '../../../system/hooks.ts';
// FIX: Corrected import path for core types.
import { HistoryEntry, GrandWorkMapResult, SelfObservationResult, HolographicAnalysis, GevurahBlueprintResult, BlueprintNode, AuthResult, NetzachAnalysis, ProphecyResult, ApiCategory, WillowCompressionResult } from '../../types.ts';
// FIX: Corrected import path for gematriaEngine.
import { gematriaEngine } from '../../core/gematria.ts';
// FIX: Corrected import path for callSigns data.
import { callSigns } from '../callsigns.data.ts';
// FIX: Corrected import path for codex.
import { codex } from '../../../codex.ts';
// FIX: Corrected import path for golem data.
import { glyphCalligrapherProject } from '../golem.data.ts';
// FIX: Corrected import path for musicEngine.
import { musicEngine } from '../../core/audio.engine.ts';
// FIX: Corrected import path for songbook data.
import { songbook } from '../songs.data.ts';
// FIX: Corrected import path for livingLibrary.
import { livingLibrary } from '../../../living-library.ts';
// FIX: Corrected import path for UNIVERSAL_CODEX_RAW data.
import { UNIVERSAL_CODEX_RAW } from '../universal_codex.data.ts';
// FIX: Corrected import path for API_CODEX_DATA.
...
const WillowCompressionComponent: React.FC<{ result: WillowCompressionResult }> = ({ result }) => {
    return (
        <div className="willow-compression-container">
            <div className="wc-header">
                <h3>{result.title}</h3>
                <p>{result.synthesis}</p>
            </div>
            <div className="wc-islands">
                {result.islands.map(islandData => (
                    <div key={islandData.island} className="wc-island">
                        <h4>Island {islandData.island} (Tier {islandData.tier})</h4>
                        <div className="wc-letters">
                            {islandData.letters.map(letter => (
                                <span key={letter.letter} className="wc-letter" style={{ color: letter.color, borderColor: letter.color }}>
                                    {letter.letter}
                                </span>
                            ))}
                        </div>
                        <p className="wc-narrative">{islandData.narrative}</p>
                        <div className="wc-checksum">Gematria Checksum: {islandData.gematriaChecksum}</div>
                    </div>
                ))}
            </div>
            <style>{`
                .willow-compression-container { border: 1px solid #C0C0C0; padding: 1rem; margin-top: 1rem; border-radius: 4px; background: rgba(20, 20, 25, 0.3); }
                .wc-header { text-align: center; border-bottom: 1px solid #C0C0C0; padding-bottom: 0.5rem; margin-bottom: 1rem; }
                .wc-header h3 { margin: 0; color: #C0C0C0; font-family: 'EB Garamond', serif; }
                .wc-header p { margin: 0.5rem 0 0; color: #bbb; font-style: italic; }
                .wc-islands { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1rem; }
                .wc-island { background: #111; border: 1px solid #333; padding: 1rem; border-radius: 3px; display: flex; flex-direction: column; }
                .wc-island h4 { color: #C0C0C0; margin: 0 0 0.75rem; }
                .wc-letters { display: flex; gap: 0.5rem; margin-bottom: 0.75rem; }
                .wc-letter { font-size: 1.5rem; font-weight: bold; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; border-width: 1px; border-style: solid; border-radius: 50%; }
                .wc-narrative { flex-grow: 1; font-size: 0.9em; color: #aaa; margin: 0 0 1rem; }
                .wc-checksum { font-family: monospace; font-size: 0.85em; color: #777; text-align: right; border-top: 1px solid #333; padding-top: 0.5rem; }
            `}</style>
        </div>
    );
};
...
const HistoryEntryComponent: React.FC<{ entry: HistoryEntry }> = ({ entry }) => {
    const renderContent = () => {
        switch (entry.type) {
            ...
            case 'WILLOW_COMPRESSION_RESULT':
                 return <WillowCompressionComponent result={entry.content} />;
            default:
